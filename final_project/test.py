...
# Least squares method to solve for the calibration matrix
...
with open('calibrated_data.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Write header row
    writer.writerow(calibration_matrix)
    writer.writerow(['Calibrated X', 'Calibrated Y', 'Calibrated Z', 'Elapsed Time'])
    ...
    while True:
        if bt_serial.in_waiting > 0:
            ...
            try:
                ...
                # Remove the effect of gravity from the z-axis reading
                calibrated_data[2] -= 9.81  # Subtract gravity from Z-axis

                # Print the corrected acceleration data in m/s²
                print(f"uncalibrated acceleration     : X={float(accel_x_str)/16384*9.81:.2f}, Y={float(accel_y_str)/16384*9.81:.2f}, Z={float(accel_z_str)/16384*9.81:.2f}")
                print(f"Calibrated Acceleration (m/s²): X={calibrated_data[0]:.2f}, Y={calibrated_data[1]:.2f}, Z={calibrated_data[2]+9.81:.2f}")

                

                calibrated_data[2] += 9.81
                temp = [float(accel_x_str)/16384*9.81, float(accel_y_str)/16384*9.81, float(accel_z_str)/16384*9.81]

                uncal_count += np.abs(np.linalg.norm(temp) - 9.81)
                cal_count += np.abs(np.linalg.norm(calibrated_data) - 9.81)

                print("uncalibrated mag:", uncal_count)
                print("calibrated   mag:", cal_count)
                ...
                print(f"Estimated Velocity (m/s): X={velocity[0]:.2f}, Y={velocity[1]:.2f}, Z={velocity[2]:.2f}")

                # Write calibrated acceleration and elapsed time to the CSV file
                writer.writerow([calibrated_data[0], calibrated_data[1], calibrated_data[2], elapsed_time])
                writer.writerow([displacement[0], displacement[1], displacement[2]])
